...
echo "🚀 Starting file cleaning process in 0-inbox..."
...
if [ "$BACKUP_ENABLED" = true ]; then
    ...
    create_directory_backup "$INBOX_DIR" "*.md" "file_cleaning"
fi
...
for file in *.md; do
  if [ -f "$file" ]; then
    ...
    awk '!/<%\*/, /-%>/' "$temp_file" > "${temp_file}.1"
    
    # Remove the "`= this.file.name`" line
    grep -v '`= this\.file\.name`' "${temp_file}.1" > "${temp_file}.2"
    
    # Remove leading and trailing empty lines
    awk 'NF {p=1} p' "${temp_file}.2" > "${temp_file}.3"
...
echo ""
echo "🎉 Processing complete! All metadata cleaned from files in 0-inbox."
echo ""
get_backup_info
echo ""
